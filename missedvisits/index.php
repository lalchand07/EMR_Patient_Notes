...
$sql = "SELECT * FROM missed_visit_notes where patient_id = $pid";
...
<head>
    ...
    <title>Missed Visit Notes | Motion Sync EMR</title>
...
<body>
    <div class="container">
        <header>
            ...
            <a class="btn btn-primary" href="addnote.php?token_main=<?php echo $_GET['token_main']."&term=".$patientData['id'] ?>"><i class="fa-solid fa-circle-plus"></i> Add New Missed Visit Note</a>
        </header>

        <h2 class="intake-title">Missed Visit Notes (<?php echo $patientData['title']. ' '. $patientData['fname'] .' '. $patientData['lname']    ?> )</h2>
        ...
        <tbody>
            ...
            if ($result->num_rows > 0) {
                while($row = $result->fetch_assoc()) {
                    ...
                    echo "<td>" . (isset($row['cancelled']) && $row['cancelled'] == "1" ? 'YES' : 'NO' )  . "</td>";
                    echo "<td>" . (isset($row['scheduled']) && $row['scheduled'] == "1" ? 'YES' : 'NO' ) . "</td>";
                    echo "<td>" . (isset($row['recheduled']) && $row['recheduled'] == "1" ? 'YES' : 'NO' ) . "</td>";
